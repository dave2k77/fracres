...
def main():
    print(f"Fractional Amari neural-field reservoir (ring of N={N}, alpha_D=0.8)\n")
    ...
    print(f"  d=0:{float(row[0]):+.3f}  d=1:{float(row[1]):+.3f}"
          f"  d=3:{float(row[3]):+.3f}  d=8:{float(row[8]):+.3f}"
          f"  d=20:{float(row[20]):+.3f}")
    ...
    peak_ratio = float(field_power[k_field] / jnp.mean(field_power[1:]))
    print(f"  kernel preferred wavenumber : k={k_kernel}  (~{N / k_kernel:.0f} sites)")
    print(f"  emergent field power peak    : k={k_field}  (~{N / k_field:.0f} sites)")
    print(f"  peak / mean band power       : {peak_ratio:.2f}x")
    ...
    for d in (1, 2, 4, 8):
        ...
        fitted = fit_readout_ridge(
            rc, drive[:SPLIT], target[:SPLIT], beta=1e-2, washout=200
        )
        Y = fitted(drive)
        corr = float(jnp.corrcoef(Y[SPLIT:, 0], target[SPLIT:, 0])[0, 1])
        print(f"{d:>6} | {corr:>10.3f}")
    print("\n(corr -> 1 = perfect recall; decays with delay = fading memory.)")
